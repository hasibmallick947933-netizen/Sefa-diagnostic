...
export default function HomePage() {
  ...
  <p className="text-sm font-semibold tracking-[0.3em] text-teal-600 uppercase mt-1 animate-pulse">
    & Polyclinic
            ...
            <div className="inline-flex items-center gap-2 bg-primary-50 border border-primary-100 text-primary-700 text-xs font-semibold px-4 py-2 rounded-full mb-6">
              <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
              NABL Accredited Diagnostic Center
            </div>
            ...
            <div className="flex flex-wrap gap-3 mb-10">
              <Link to="/book-appointment" className="btn-primary flex items-center gap-2 text-base">
                Book Appointment <ArrowRight size={18} />
              </Link>
              <Link to="/tests" className="btn-outline flex items-center gap-2 text-base">
                Book a Test
              </Link>
            </div>
              ...
              <div className="card p-6 w-full max-w-sm shadow-2xl">
                ...
                <div className="space-y-3 mb-5">
                  {[
                    ...
                    <div key={item.name} className="flex items-center gap-3 p-3 bg-neutral-50 rounded-xl">
                      <span className="text-xl">{item.icon}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-neutral-700 truncate">{item.name}</p>
                        <p className="text-[10px] text-neutral-400">{item.time}</p>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {SERVICES.map(service => (
              <div key={service.title} className="card p-5 text-center cursor-default group hover:-translate-y-1 transition-transform duration-300">
                <div className="text-3xl mb-3">{service.icon}</div>
                <h3 className="font-semibold text-neutral-800 text-sm mb-1">{service.title}</h3>
                <p className="text-xs text-neutral-400">{service.desc}</p>
              </div>
      ...
      <section className="py-20 bg-neutral-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {DOCTORS.slice(0, 3).map(doc => (
              <div key={doc.id} className="card p-5 group hover:-translate-y-1 transition-transform duration-300">
                <div className="flex items-start gap-4 mb-4">
                  <img
                    src={doc.image}
                    alt={doc.name}
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {HEALTH_PACKAGES.map(pkg => (
              <div key={pkg.id} className={`card p-6 relative ${pkg.popular ? 'ring-2 ring-primary-500' : ''}`}>
                {pkg.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="bg-primary-600 text-white text-xs font-bold px-4 py-1 rounded-full">MOST POPULAR</span>
                  </div>
      ...
      <section className="py-16 bg-gradient-to-r from-primary-700 to-teal-600 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center relative z-10">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <Link to="/book-appointment" className="bg-white text-primary-700 hover:bg-primary-50 font-bold px-8 py-3.5 rounded-xl transition-colors shadow-lg">
              Book Appointment
            </Link>
            <a href="https://wa.me/919876543210?text=Hi, I'd like to book a test at Sefa Diagnostic Centre" target="_blank" rel="noopener noreferrer"
              className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold px-8 py-3.5 rounded-xl transition-colors shadow-lg flex items-center gap-2"
            >
              <Phone size={18} /> WhatsApp Booking
            </a>
